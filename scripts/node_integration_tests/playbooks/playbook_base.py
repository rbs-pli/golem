...
class NodeTestPlaybook:
    ...
    nodes: 'typing.Dict[NodeId, Popen]'
    output_queues: 'typing.Dict[NodeId, Queue]'
    nodes_ports: typing.Dict[NodeId, int]
    nodes_keys: typing.Dict[NodeId, typing.Any]
    nodes_exit_codes: typing.Dict[NodeId, typing.Optional[int]]
    ...
    def step_configure_node(self, node_id: NodeId) -> None:
    ...
    def step_verify_peer_connection(self, node_id: NodeId, target_node: str):
        def on_success(result):
            if len(result) > 1:
                ...
                if peer['key_id'] != expected_peer_key:
                    print("Connected peer: {} != expected peer: {}",
                          peer.key, expected_peer_key)
                    self.fail()
                    return
        ...
        return self.call(node_id, 'net.peers.connected',
                         on_success=on_success)

    def step_get_nodes_known_tasks(self, node_id: NodeId):
        def on_success(result):
    ...
    def step_create_task(self, node_id: NodeId):
        ...
        if not self.task_in_creation:
            ...
            return self.call(node_id, 'comp.task.create', self.task_dict,
                             on_success=on_success)
    ...
    initial_steps: typing.Tuple = (
        ...
        partial(step_get_nodes_known_tasks, node_id=NodeId.requestor),
    )
    ...
    def call(self, node_id: NodeId, method, *args,
             on_success=_print_result,
    ...
    def start_nodes(self):
        for node_id, node_config in self.config.current_nodes.items():
            print(f"{node_id.value} data directory: {node_config.datadir}")
    ...
    def __init__(self, config: 'Config') -> None:
        ...
        self.task_dict = helpers.construct_test_task(
            task_package_name=self.config.task_package,
            output_path=self.output_path,
            task_settings=self.config.task_settings,
        )
